...
export async function storeTopicsInNeo4j(endpoint: string, formattedDate: string, partition: number) {
  const prefix = await g.getListPrefix(endpoint, KeyTypeDiscourse.latest, formattedDate, partition)
...
async function processKey(key: string, endpoint: string): Promise<DiscourseNeo4jTopic[]> {
  console.log(key)
  const data = await s.get(key) as DiscourseRawLatest
  return data.topic_list.topics.map(topic => t.transform(topic, { endpoint }))
}