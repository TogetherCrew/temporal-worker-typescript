...
describe('verifyTelegram', () => {
  ...
  it('should verify successfully when one match platform is found', async () => {
    ...
    expect(mockPlatform.set).toHaveBeenCalledWith("metadata.token.verifiedAt", expect.any(Date));
  ...
  it("should return a failure message when multiple matching platforms are found", async () => {
    (Platform.find as jest.Mock).mockResolvedValue([mockPlatform, mockPlatform]); // Simulate multiple matching platforms
  ...
  it("should throw an error if `setVerified` encounters an issue", async () => {
    ...
    // Act & Assert
    await expect(verifyTelegram(token, chat, from)).rejects.toThrow("Save failed");
